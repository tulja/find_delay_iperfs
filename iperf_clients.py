...
def start_iperf_client(num): 
	...
	"""
	print("Starting the iperf client session on port 50"+str(num)) 
	server_ip = "10.0.0.2"
	os.system("iperf3 -c "+server_ip+" -t 20 -p 50"+str(num)+"")


def start_netperf():
	global output
	cmd = "netperf -P 0 -l 18 -t TCP_RR -H \"10.0.0.2\" -- -r 1,1 -o P50_LATENCY,P90_LATENCY,P99_LATENCY,MIN_LATENCY,STDDEV_LATENCY,MEAN_LATENCY,MAX_LATENCY"
...
#	f= open("guru99.txt","w+")
	...
	#netperf_threads = [0]*(n_threads+1)
#	output = [0]*(n_threads+1)
	...
	for x in xrange(1,n_threads):
		...
		#netperf_threads[x] = threading.Thread(target=start_netperf, args=(x,)) 

	for x in xrange(1,n_threads):
		all_threads[x].start()
		#netperf_threads[x].start()
		
	for x in xrange(1,n_threads):
		all_threads[x].join()
		#netperf_threads[x].join()
	...
	#for x in xrange(1,n_threads):
	print("Latency observed as "+str(output))
	with open('latencies.dat', 'a') as the_file:
